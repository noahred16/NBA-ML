...
WITH game_matchups AS (
	SELECT
		...
		G.pts_away, -- target 2
		G.wl_home, -- target 3
	-- MatchUpStats (Sequential Data)
		-- Sequential Label Data
		M.game_id AS 'matchup_game_id',
		DATE(M.game_date) AS 'matchup_game_date', -- label data as we can use days ago as a more valuable feature
		M.season_type AS 'matchup_season_type',
		...
		CASE WHEN M.team_id_home = G.team_id_home THEN 1 ELSE 0 END AS 'is_home_game_matchup',
		CASE WHEN M.season_type = 'Regular Season' THEN 1 ELSE 0 END AS 'is_regular_season_matchup',
		CASE WHEN M.season_type = 'Playoffs' THEN 1 ELSE 0 END AS 'is_playoffs_matchup',
		CASE WHEN M.season_type = 'Pre Season' THEN 1 ELSE 0 END AS 'is_pre_season_matchup',
	...
	FROM
		game G
	JOIN game M ON -- grab matchups (M) for each game (G)
		M.team_id_home IN (G.team_id_away, G.team_id_home) -- search for prev matchups
		AND M.team_id_away IN (G.team_id_away, G.team_id_home)
	...
	WHERE CAST(strftime('%Y', G.game_date) AS INTEGER) > 2000 -- Only care about games after 2000
		...
		AND M.game_date > DATE(G.game_date, '-1825 days') -- and within 365days*5years = 1825 days of the game being analyzed
--		AND G.game_id IN (SELECT game_id FROM game ORDER BY game_id	LIMIT (10000)) -- limit for faster query, assume each teams plays once a year
)
SELECT *
FROM game_matchups 
WHERE games_ago <= 10 -- Limit for sequential data
	 AND game_id IN (SELECT DISTINCT game_id FROM game_matchups WHERE games_ago = 5) -- filter out games with less than 5 matchups
ORDER BY game_date, game_id, matchup_game_date DESC